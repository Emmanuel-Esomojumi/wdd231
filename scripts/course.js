...
function renderCourses(list) {
    courseList.innerHTML = list
        .map(
            (course) => `
      <li class="course-card ${course.completed ? "is-completed" : ""}" title="${course.title} — ${course.credits} credits">
        ${course.completed ? '<span class="course-card__check" aria-hidden="true">&#10003;</span>' : ""}
        <span class="course-card__code">${course.subject} ${course.number}</span>
        <span class="visually-hidden">${course.completed ? "completed" : "not yet completed"}, ${course.title}, ${course.credits} credits</span>
      </li>
    `
        )
        .join("");
